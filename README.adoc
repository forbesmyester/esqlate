...
Installation is relatively simple however there are a few parts.

* https://www.postgresql.org/[PostgreSQL] The most awesome SQL database - somewhere.
* https://github.com/forbesmyester/wv-linewise[WVLinewise] - Put a the binary somewhere in your $PATH named `wv_linewise`
* https://github.com/forbesmyester/pipeawesome2[Pipeawesome 2] - Put a the binary somewhere in your $PATH named `pipeawesome2`
* Configure environmental variables to access PostgreSQL. See Guided method - Terminal 2.
* Checkout ( https://github.com/forbesmyester/esqlate[this repository] ) and change to this directory.
* Run `pipeawesome2 process --config ./esqlate.pa.yaml
...
export LISTEN_ONLY_LOCALHOST=1
----
...
[source,bash]
----
export API_SERVER=http://localhost:8803
export LISTEN_PORT=8804
----
...
There are some basic integration tests that run using http://www.cypress.io[Cypress]. To run them first bring up testing docker compose and then run the tests with `node_modules/.bin/cypress run`:
